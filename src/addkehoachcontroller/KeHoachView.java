...
public class KeHoachView extends JPanel  {
	private JTable table;
	private JLabel lbName;
	private JScrollPane scroll;
	private JButton btNext,btBack;
	private int currentPage;
	private String[] columns = { "Vòng đấu", "Giải Đấu","NGày đấu", "Đại điểm", "Đối Thủ ","Trạng Thái"};
...
//	}
	...
	private JPanel namePanel() {
		...
		//create label
				lbName = new JLabel("DANH SÁCH Kế Hoạch");
	...
	public void updateTable(List<KeHoach> list){
		...
		for(int i=0;i<list.size();i++){
			KeHoach nv      =    list.get(i);
			data[i][0] 	=   String.valueOf(nv.getVongdau());
                        data[i][1]      =   nv.getGiaidau();
			data[i][2] 	= nv.getNgaydau();
			data[i][3]     =   nv.getDiadiem();
			data[i][4] 	=    nv.getDoithu();
	...
	public void setCurrentPage(int currentPage) {
        ...
        new KeHoachView().setVisible(true);
    }
}
